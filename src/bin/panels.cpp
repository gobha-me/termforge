...
auto uptime_text(double seconds) -> std::string {
  ...
  if (days > 0)
    return std::format("{}d {:02}:{:02}", days, hours, minutes);
...
OverviewPanel::OverviewPanel() { m_frame.set_style(BorderStyle::Rounded); }
...
auto CpuPanel::pixel_region_submitted(Rect region) noexcept -> void {
  ...
  for (std::size_t i = 0; i < waves.size(); ++i)
    if (waves[i]->rect() == region) {
      ...
      if (i < m_active_invalidated.size())
        m_active_invalidated[i] = false;
      return;
    }
}
  ...
  m_swap.set_label(memory.swap_total_bytes == 0
                       ...
                       : [&] {
                           ...
                           return std::format(
                               ...
                               scaled_bytes(memory.swap_free_bytes, scale),
                               scale.unit);
                         }());
  mark_dirty();
}
...
auto ProcessPanel::handle_header_click(const MouseEvent &mouse) -> bool {
  ...
  const auto sort_for = [](ProcessColumn column) {
    switch (column) {
    case ProcessColumn::Pid:
      return ProcessSort::Pid;
    case ProcessColumn::User:
      return ProcessSort::User;
    case ProcessColumn::State:
      return ProcessSort::State;
    case ProcessColumn::Cpu:
      return ProcessSort::Cpu;
    case ProcessColumn::Memory:
    case ProcessColumn::Resident:
      return ProcessSort::Memory;
    case ProcessColumn::Time:
      return ProcessSort::Time;
    case ProcessColumn::Command:
...
auto ProcessPanel::update_columns(int width) -> void {
  ...
  if (user)
    m_columns.push_back(ProcessColumn::User);
  if (state)
    m_columns.push_back(ProcessColumn::State);
  m_columns.push_back(ProcessColumn::Cpu);
  m_columns.push_back(ProcessColumn::Memory);
  if (time)
    m_columns.push_back(ProcessColumn::Time);
  if (resident)
    m_columns.push_back(ProcessColumn::Resident);
...
auto ProcessPanel::rebuild() -> void {
  ...
  for (const auto &process : m_processes) {
    const std::string haystack = ascii_lower(std::format(
        "{} {} {} {}", process.pid, process.user, process.name,

...
TEST_CASE("char_width: combining, zero-width and controls are zero",
          "[width]") {
...
TEST_CASE("utf8_decode: recovers scalar value and byte length",
          "[width][utf8]") {
  char32_t cp = 0;
  std::size_t len = 0;
...
TEST_CASE("truncate_to_width: never straddles a wide glyph",
          "[width][failure]") {
  const std::string shijie =
      std::string(kShi) + std::string(kJie); // 世界 (4 cols)
